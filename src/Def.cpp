...
// #define DEBUG
namespace gazprea {
Def::Def(std::shared_ptr<SymbolTable> symTab, std::shared_ptr<int> mlirID)
    : symtab(symTab)
    , varID(mlirID)
{
...
std::any Def::visitProcedure(std::shared_ptr<ProcedureNode> tree)
{
    ...
    if (tree->body) {
        ...
        if (tree->getRetTypeNode()) { // has return
            ...
            if (retType == nullptr)
                throw TypeError(tree->loc(), "cannot verify types");
        }
