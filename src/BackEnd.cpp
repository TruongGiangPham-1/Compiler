...
void BackEnd::setupCommonTypeRuntime()
{
    ...
    auto commonType = mlir::LLVM::LLVMStructType::getLiteral(&context, { intType, intPtrType });

    auto commonTypeAddr = mlir::LLVM::LLVMPointerType::get(commonType);

    auto listType = mlir::LLVM::LLVMStructType::getLiteral(&context, { intType, intType, mlir::LLVM::LLVMPointerType::get(commonTypeAddr) });
    ...
    auto allocateCommonType = mlir::LLVM::LLVMFunctionType::get(commonTypeAddr, { voidPtrType, intType });
    auto allocateFromRange = mlir::LLVM::LLVMFunctionType::get(commonTypeAddr, { commonTypeAddr, commonTypeAddr });
    ...
    auto deallocateCommonType = mlir::LLVM::LLVMFunctionType::get(voidType, commonTypeAddr);
    ...
    builder->create<mlir::LLVM::LLVMFuncOp>(loc, "appendCommon",
        appendCommon);
    builder->create<mlir::LLVM::LLVMFuncOp>(loc, "appendStack",
        appendCommon);
...
{
    mlir::LLVM::LLVMFuncOp binopFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("performCommonTypeBINOP");

    auto result = builder->create<mlir::LLVM::CallOp>(loc,
                             binopFunc,
                             mlir::ValueRange({ left,
                                 right,
                                 generateInteger(op) }))
                      .getResult();
...
{
    std::string newLabel = "OBJECT_NUMBER" + std::to_string(this->allocatedObjects);
...
{
    mlir::LLVM::LLVMFuncOp promotionFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("indexCommonType");
...
{
    mlir::LLVM::LLVMFuncOp copyFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("copyCommonType");
...
{
    mlir::LLVM::LLVMFuncOp normalizeFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("normalize");
...
{
    mlir::LLVM::LLVMFuncOp promotionFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("promotion");
...
{
    mlir::LLVM::LLVMFuncOp promotionFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("cast");
...
{
    mlir::LLVM::LLVMFuncOp promotionFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("castHelper");
...
{
    mlir::LLVM::LLVMFuncOp appendFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("appendCommon");
...
void BackEnd::appendStack(mlir::Value destination, mlir::Value item)
{
    mlir::LLVM::LLVMFuncOp appendFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("appendStack");
...
void BackEnd::printCommonType(mlir::Value value)
{
    ...
    mlir::LLVM::LLVMFuncOp printVecFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("printCommonType");
...
{
    mlir::LLVM::LLVMFuncOp streamOutFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("streamOut");
...
void BackEnd::streamIn(mlir::Value value)
{
    ...
    mlir::LLVM::LLVMFuncOp streamInFunc = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("streamIn");
...
mlir::Value BackEnd::generateCommonType(mlir::Value value, int type)
{
    ...
    mlir::LLVM::LLVMFuncOp allocateCommonType = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("allocateCommonType");
...
{
    mlir::LLVM::LLVMFuncOp deallocate = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("deallocateStack");
...
void BackEnd::deallocateObjects()
{
    for (auto label : **(objectLabels.end() - 1)) {
        mlir::LLVM::LLVMFuncOp deallocateObject = module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("deallocateCommonType");
...
void BackEnd::generateInitializeGlobalVar(std::string varName,
    mlir::Value value)
{
    mlir::Block* currentBlock = builder->getBlock(); // get current block we are in so we can return here
                                                     // later
...
{
    mlir::Value one = builder->create<mlir::LLVM::ConstantOp>(loc, builder->getI32Type(), 1);
...
void BackEnd::setBuilderInsertionPoint(
    mlir::Block* block)
{ // set insertion point but no smart pointer:
