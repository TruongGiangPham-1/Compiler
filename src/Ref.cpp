...
// #define DEBUG
namespace gazprea {
Ref::Ref(std::shared_ptr<SymbolTable> symTab, std::shared_ptr<int> mlirIDptr)
    : symtab(symTab)
    , varID(mlirIDptr)
{
...
std::any Ref::visitFunction(std::shared_ptr<FunctionNode> tree)
{
    ...
    if (funcSym == nullptr) { //  can't resolve means that there was no forward declaration
        ...
        if (retType == nullptr)
            throw TypeError(tree->loc(), "cannot resolve function return type");
        ...
        if (std::dynamic_pointer_cast<FunctionSymbol>(funcSym)) {
            ...
            if (tree->getRetTypeNode()) {
                ...
                if (retType == nullptr)
                    throw TypeError(tree->loc(), "cannot resolve functin return type");
            }
            ...
            if (find == this->funcProtypeList.end())
                return 0; // there was forward declaration but the prototype appear after this defintino
...
std::any Ref::visitProcedure(std::shared_ptr<ProcedureNode> tree)
{
    ...
    if (procSym == nullptr) { //  can't resolve means that there was no forward declaration
        ...
        if (tree->getRetTypeNode()) {
            retType = symtab->resolveTypeUser(tree->getRetTypeNode());
            if (retType == nullptr)
                throw TypeError(tree->loc(), "cannot resolve procedure return type");
        ...
        if (std::dynamic_pointer_cast<ProcedureSymbol>(procSym)) {
            ...
            if (find == this->procProtypeList.end())
                return 0; // this definition is higher in the file then forward declaration so we dont need to swap
...
{
    if (tree->domainVar1 == tree->domainVar2)
        throw SymbolError(tree->loc(), "redefinition of domainVar");
    int isVec = 1;
    ...
    if (isVec) {
        ...
        std::cout << "in line " << tree->loc()
                  ...
                  << domainVar2Sym->mlirName << std::endl;
#endif
    }
